...
public class WorkingWithEvents
{
    static void Main(string[] args)
    {
        ...
        WarehouseProductHandler.DemoConsole.WriteWithColour(
            $"{warehouseProductReadModel?.QuantityOnHand ?? 0} items of stock in the warehouse for {id}");

        var handler = new WarehouseProductHandler(id, documentStore);
        handler.ReceiveProduct(100);

        WarehouseProductHandler.DemoConsole.WriteWithColour($"Received 100 items of stock into the warehouse for {id}");

        handler.ShipProduct(10);

        WarehouseProductHandler.DemoConsole.WriteWithColour($"Shipped 10 items of stock out of the warehouse for {id}");

        handler.AdjustInventory(5, "Ordered too many");

        WarehouseProductHandler.DemoConsole.WriteWithColour(
            $"Found 5 items of stock hiding in the warehouse for {id} and have adjusted the stock count");

        warehouseProductReadModel = warehouseRepository.Get(id);

        WarehouseProductHandler.DemoConsole.WriteWithColour(
            $"{warehouseProductReadModel.QuantityOnHand} items of stock in the warehouse for {warehouseProductReadModel.Id}");
    }
}
